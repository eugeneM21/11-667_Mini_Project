...
def calculate_similarity_metrics(generated_text, reference_text):
    """
    Computes similarity metrics for generated and reference text.
    """
    metrics = {}
    ...
    # 3. BERTScore
    bert_p, bert_r, bert_f1 = bert_score([generated_text], [reference_text], lang="en", verbose=False)
    metrics["BERTScore"] = bert_f1.mean().item()
    print(f'BERTScore: {metrics["BERTScore"]}')

    # 4. SBERT Similarity
    sbert_model = SentenceTransformer('all-MiniLM-L6-v2')
    sbert_generated = sbert_model.encode(generated_text, convert_to_tensor=True)
    sbert_reference = sbert_model.encode(reference_text, convert_to_tensor=True)
    metrics["SBERT"] = util.cos_sim(sbert_generated, sbert_reference).item()
    print(f'SBERT: {metrics["SBERT"]}')

    # 5. USE (Universal Sentence Encoder)
    use_model = SentenceTransformer('sentence-transformers/all-distilroberta-v1')
    use_generated = use_model.encode(generated_text, convert_to_tensor=True)
    use_reference = use_model.encode(reference_text, convert_to_tensor=True)
    metrics["USE"] = util.cos_sim(use_generated, use_reference).item()
    print(f'USE: {metrics["USE"]}')

    # 6. TF-IDF Cosine Similarity
    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform([generated_text, reference_text])
    metrics["TF-IDF Cosine"] = cosine_similarity(tfidf_matrix[0:1], tfidf_matrix[1:2]).flatten()[0]
    print(f'TF-IDF Cosine: {metrics["TF-IDF Cosine"]}')

    # 7. Jaccard Similarity
    # Tokenize and create sets of words
    set1 = set(generated_text.split())
...
def calculate_perplexity(model, tokenizer, dataset_path, max_length=512):
    ...
    for idx, row in val_df.iterrows():
        ...
        with torch.no_grad():
            ...
            similarity_metrics = calculate_similarity_metrics(generated_text, reference_text)
